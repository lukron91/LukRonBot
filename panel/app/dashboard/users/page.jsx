...
export default function UsersPage() {
  ...
  return (
    <div className="up-root">
      ...
      {/* Modal profilu */}
      {selectedUser && (
        <div className="up-modal-overlay" onClick={closeProfile}>
          <div className="up-modal" onClick={e => e.stopPropagation()}>
            {/* Nagłówek */}
            <div className="up-modal-header">
              <button className="up-close" onClick={closeProfile}><FiX /></button>
              <div className="up-modal-tabs">
                  ...
                  <div className="up-profile-top">
                    ...
                    <div className="up-profile-info">
                      ...
                      <div className="up-profile-badges">
                        {profile.isOwner && <span className="up-badge owner">WŁAŚCICIEL</span>}
                      ...
                      <div className="up-punish-form">
                        ...
                        {punishResult && (
                          <div className={`up-punish-result ${punishResult.success ? 'success' : 'error'}`}>
                            {punishResult.success ? '✅ ' : '❌ '}{punishResult.message}
                          </div>
                        )}
                      </div>
                    )}
      ...
      <style jsx>{`
        ...
        /* Modal */
        .up-modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.7); z-index: 1000; display: flex; align-items: center; justify-content: center; padding: 1rem; }
        .up-modal { background: var(--surface-color); border: 1px solid var(--border-color); border-radius: var(--border-radius); width: 100%; max-width: 560px; max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 32px 80px rgba(0,0,0,0.6); }
        .up-modal-header { border-bottom: 1px solid var(--border-color); position: relative; }
        .up-close { position: absolute; top: 0.75rem; right: 0.75rem; background: none; border: none; color: var(--text-muted); cursor: pointer; font-size: 1.1rem; z-index: 1; }
        .up-close:hover { color: var(--text-color); }
        ...
        /* Ukryj domyślne strzałki input type=number */
